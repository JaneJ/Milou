 ...
 */
@WebServlet
public class PiltSocketController extends WebSocketServlet implements WebSocketCreator {

    private List<PiltSocket> sockets;
    private ServletContext context;

    
 
    public void broadcast(String message) {
        for (PiltSocket socket : sockets) {
            try {
                socket.send(message);
            } 
            catch (IOException e) {
    ...
    public List<PiltSocket> getSockets() {
        return sockets;
    }
    ...
    public void init(ServletConfig config) throws ServletException {
        super.init(config);
        sockets = new CopyOnWriteArrayList<PiltSocket>(); // thread-safe impl
    ...
    public Object createWebSocket(ServletUpgradeRequest req, ServletUpgradeResponse resp) {
        return new PiltSocket(this); // socket instance created per client
    }
    ...
    private static void publish(PiltSocketController controller, ServletContext context) {
        // see @WebListener and @WebFilter for details about servlet init
        context.setAttribute(PiltSocketController.class.getName(), controller);
    }

    public static PiltSocketController find(ServletContext context) {
        return (PiltSocketController) context.getAttribute(PiltSocketController.class.getName());
    }
